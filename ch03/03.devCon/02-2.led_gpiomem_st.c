...
#define	DEVICE_FILENAME	"/dev/gpiomem"
...
int main(void)
{
	...
	if(fd < 0) {
		perror("/dev/gpiomem file open error!!");
		exit(-1);
	} else printf("/dev/gpiomem file open ok!!\n");

	gpio_map = _______(NULL, GPIO_SIZE, PROT_READ|PROT_WRITE, 
				MAP_SHARED, _____, 0);
